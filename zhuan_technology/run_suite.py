# 加载每个接口类的测试用例，并执行
from zhuan_technology import zhuanzhuan
...
suite.addTest(loader.loadTestsFromModule(zhuanzhuan))  # 加载模块下的所有测试用例类
...
with open("test_report.html", 'wb') as file:  # 如果有乱码需要加一个encoding='UTF-8'，此方法打开文件后需要关闭文件
    runner = HTMLTestRunnerNew.HTMLTestRunner(stream=file, verbosity=2, title="新也买小程序接口自动化测试报告",
